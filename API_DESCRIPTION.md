...
| author_name | string   | Имя автора                  | `admin`         |
...
Набор привелегий определяется битовой маской (целым числом).
Чтобы составить нужный набор привелегий, нужно сложить значения нужных привелегий.
Например чтобы получить привелегии admin и all_channels нужно сложить 1 и 2.
Значение привелегии = 2^(Бит).

| Привилегия   | Бит | +   | Описание                                   |
| ------------ | --- | --- | ------------------------------------------ |
| admin        | 0   | 1   | Возможность совершать почти любые действия |
| all_channels | 1   | 2   | Позволяет видеть все доступные каналы      |
| colored      | 2   | 4   | Сообщения в чате выделяются цветом         |
...
Проверка валидности токена. Также возвращает ID владельца.
...
Завершение сессии по переданному ключу сессии.
...
**/api/exit_all_sessions**

Завершение всех открытых сессий данного полльзователя.
...
| token   | string | Ключ сессии     | `"aS31L42n21hD"` |
| user_id | int    | ID пользователя | `1`              |
| avatar  | string | Файл аватара    | бинарный файл    |
...
| Имя      | Тип   | Описание                                             | Пример                                    |
| -------- | ----- | ---------------------------------------------------- | ----------------------------------------- |
| channels | Array | Массив объектов каналов (см. "Используемые объекты") | `[{"id":1,"name":"IU9-23 chat",...},...]` |
